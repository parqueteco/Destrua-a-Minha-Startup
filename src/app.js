...
// ==========================================
// A API_KEY será substituída pelo Vite durante o build graças ao vite.config.ts
const API_KEY = process.env.GEMINI_API_KEY || '';

// ==========================================
// 2. INICIALIZAÇÃO À PROVA DE FALHAS
// ==========================================
// Envolvemos TODO o código neste evento para garantir que o JavaScript 
// só tente procurar os botões DEPOIS que o HTML estiver 100% renderizado.
document.addEventListener('DOMContentLoaded', () => {
    
    console.log("Interface carregada com sucesso! Conectando as engrenagens...");
    ...
    // ==========================================
    // 4. LÓGICA DE ESTADO (Funções visuais)
    // ==========================================
    function setLoading(isLoading) {
        if (!btnDestruir) return; 
        if (isLoading) {
    ...
    // ==========================================
    // 5. INTEGRAÇÃO COM GEMINI API
    // ==========================================
    async function analyzeStartup() {
        if (!API_KEY || API_KEY === 'SUA_API_KEY_AQUI') {
            alert("ERRO: Chave da API do Gemini não configurada corretamente.");
            return;
        }
        ...
        if (isUrlMode) {
            ...
            try {
                ...
                if (!jinaResponse.ok) throw new Error("Falha no Jina AI");
                scrapedText = await jinaResponse.text();
            } catch (jinaError) {
                console.warn("Falha ao extrair texto:", jinaError);
                scrapedText = "[ERRO: Faça a análise baseada na URL fornecida]";
            }
        ...
        try {
            ...
            alert("Ocorreu um erro ao tentar destruir esta startup. Tente novamente.");
        } finally {
            setLoading(false);
        }
    }
    ...
    // ==========================================
    // 6. ADICIONANDO OS EVENT LISTENERS (CLIQUES)
    // ==========================================
    // Somente ativamos as funções se a aba URL existir (confirmando que o DOM carregou bem)
    if (tabUrl) {
        ...
        btnDestruir.addEventListener('click', () => analyzeStartup());

        inputUrl.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') analyzeStartup();
        });

        btnVoltarTop.addEventListener('click', resetApp);
        btnReset.addEventListener('click', resetApp);
    } else {
        console.error("ERRO: Elemento 'tab-url' não foi encontrado. Verifique se o ID está correto no seu HTML.");
    }
